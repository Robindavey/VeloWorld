# VeloWorld — Physics Model Specification

## Purpose

This document defines the physics model used in the VeloWorld simulation. The objective is to accurately simulate the forces acting on a cyclist moving through a real-world environment, and to translate those forces into smart trainer resistance commands in real time.

VeloWorld's physics model is designed to be the most accurate cycling dynamics simulation available in any consumer indoor training platform. Unlike platforms that use simplified gradient-only resistance models, VeloWorld computes all significant forces every simulation frame.
