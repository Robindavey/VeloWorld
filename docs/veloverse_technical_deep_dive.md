# VeloVerse — Technical Deep Dive

## Purpose

This document provides a detailed technical reference for the VeloVerse system. It is intended for engineers working on the platform and covers all major technical decisions, design patterns, interfaces, and engineering challenges in depth.
...
VeloVerse is composed of four primary technical systems:
...
**Nanite Virtualised Geometry**: Enables rendering of extremely high-polygon terrain meshes (generated from 1m LiDAR data) without manual LOD management. Critical for VeloVerse's terrain quality requirements.
...
The pipeline is the core technical innovation of VeloVerse. It runs server-side as an asynchronous job system. Each pipeline run processes one route and produces a packaged simulation environment.
  ...
  "chunks": [
    {
      "start_m": 0,
      "end_m": 500,
      "terrain_tile": "s3://veloverse/routes/abc-123/terrain/tile_0000.glb",
      "road_section": "s3://veloverse/routes/abc-123/road/section_0000.glb",
      "assets": ["s3://veloverse/assets/trees/alpine_01.glb", ...]
    },
    ...
  ]
}
...
VeloVerse uses **Set Indoor Bike Simulation Parameters** as the primary command. This delegates the force curve to the trainer's internal model, which is tuned for its specific flywheel. The grade value is set to the equivalent gradient from the physics model:
...
VeloVerse uses **Page 51** (Track Resistance) for free riding and **Page 49** (Target Power) for ERG mode.
